...
corp_code_input = st.sidebar.text_input("분석할 한국 상장사 6자리 종목코드 입력 (예: 005930, 000270)", "005930")
bsns_year = st.sidebar.selectbox("분석 회계 연도 선택", ["2024", "2023", "2022"], index=0)

# 금감원 6자리 종목코드를 DART 고유번호(8자리)로 실시간 변환하는 마법의 함수
@st.cache_data
def get_dart_corp_code(api_key, target_code):
    try:
        url = "https://fss.or.kr"
        params = {'crtfc_key': api_key}
        response = requests.get(url, params=params)
        
        # DART 고유번호 고유의 ZIP 파일 압축 해제 및 XML 파싱
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            for list_tag in tree.findall('list'):
                ...
                if stock_code and stock_code.strip() == target_code.strip():
...
if st.sidebar.button("📊 실시간 리스크 스크리닝 시작"):
    if not api_key:
        ...
        try:
            with st.spinner("금융감독원 DART 데이터베이스 동기화 및 실시간 재무 분석 중..."):
                # 6자리 상장 종목코드를 8자리 DART 고유번호로 변환
                real_corp_code = get_dart_corp_code(api_key, corp_code_input)
                
                if not real_corp_code:
                    st.error("❌ 유효한 6자리 상장 종목코드가 아니거나 DART 고유번호 매칭에 실패했습니다. (예: 삼성전자는 005930)")
                else:
                    ...
                    if res_data.get('status') != '000':
                        st.error(f"❌ DART 서버 메시지: {res_data.get('message')} (해당 연도 데이터가 공시되지 않았을 수 있습니다.)")
                    else:
                        ...
                        if op_income is not None and total_assets is not None and total_equity is not None:
                            ...
                            if interest_expense and interest_expense > 0:
                                ...
                                interest_coverage = 999.0 # 무차입 경영 우량기업 방어막
